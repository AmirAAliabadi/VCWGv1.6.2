...
Developed by Xuan Chen, Mohsen Moradi, and Amir A. Aliabadi
...
class UWG(object):
    ...
    # UWG Class, definition of all variables, with addition of optimization variables, except for R value, which is chaned in the CSV schedules
    def __init__(self, ene_mode, opt_A_PV, opt_A_WT, opt_Infil, opt_Vent, opt_AlbRoof, opt_Glz, opt_SHGC,
                 opt_V_BITES, opt_A_ST, opt_m_dot_st_f, opt_m_dot_he_st, opt_T_melt, opt_V_pcm,
                 epwFileName,month_name, uwgParamFileName=None, epwDir=None, uwgParamDir=None, destinationDir=None, destinationFileName=None):
        ...
        self.month_name = month_name                #Name of the Specific Month/Initialization File Under Processing
        ...
        self.albRoof = opt_AlbRoof              # Roof albedo [-]
        self.alb_roof = opt_AlbRoof             # Roof albedo [-]
        self.glzR = opt_Glz                     # Glazing Ratio [-]
        self.vent = opt_Vent                    # Ventilation rate [m^3 s^-1 m^-2]
        self.infil = opt_Infil                  # Iinfiltration rate Air Change per Hour (ACH) [hr^-1]
        self.shgc = opt_SHGC                    # SHGC [-]
        self.A_wt = opt_A_WT                    # Wind turbine area per building footprint area [m2 m-2]
        self.A_pv = opt_A_PV                    # PV area per building footprint area [m2 m-2]

        self.V_bites = opt_V_BITES              # Volume of BITES per building footprint area
        self.A_st = opt_A_ST                    # Area of ST per building footprint area (half roof or not?)
        self.m_dot_st_f = opt_m_dot_st_f        # Mass flow rate of working fluid in ST
        self.m_dot_he_st = opt_m_dot_he_st      # Mass flow rate of air in ST heat exchanger
        self.T_melt = opt_T_melt                # Melting temperature f PCM
        self.V_pcm = opt_V_pcm                  # Volume of PCM per building footprint area
       ...
       # Define optional Building characteristics
        ...
        self.hvac = None              # HVAC TYPE; 0 = Fully Conditioned; 1 = Mixed Mode Natural Ventilation and Conditioned
        ...
        self.Adv_ene_heat_mode = ene_mode
        self.beta_st = None  # ST tilt angle

        self.U_st = None  # Loss coefficient of ST
        self.FR_st = None  # Heat removal factor of ST
        self.tau_alpha_e_st = None  # Effective transmittance abosorptance of ST
        self.eta_he_st = None  # Heat exchange efficiency of ST (fluid of air)

        self.c_bites = None  # Volumetric heat capacity of BITES

        self.c_st_f = None  # Heat capacity of working fluid in ST

        self.beta_pv = None
        self.eta_pv = None
        self.COP_hp_min = None  # Minimum COP_hp of auxiliary HP at minimum temperature
        self.COP_hp_max = None  # Maximum COP_hp of auxiliary HP at maximum temperature
        self.T_hp_min = None  # Minimum temperature of auxiliary HP
        self.T_hp_max = None  # Maximum temperature of auxiliary HP
    ...
    def read_input(self):
        ...
        if self.T_melt is None: self.T_melt = ipd['T_melt'] # thi # this function

        #Window Parameters
        if ipd['XWindowWidth'] != 0: self.windowList.append(Window(ipd['XWindowWidth'],ipd['XWindowHeight'],ipd['XWindowZCoord'],ipd['WindowEffectiveness'],'X',ipd['XWindowOpenArea'],ipd['XWindowScreen']))
        if ipd['YWindowWidth'] != 0: self.windowList.append(Window(ipd['YWindowWidth'],ipd['YWindowHeight'],ipd['YWindowZCoord'],ipd['WindowEffectiveness'],'Y',ipd['YWindowOpenArea'],ipd['YWindowScreen']))
    def set_input(self):
        ...
        if self.uwgParamFileName is not None:
            # Print statement to show that and EPW file is being read
            # print "\nReading uwg file input."
            self.read_input()
        else:
            # Print statement if no EPW file can be read
            print "\nNo .uwg file input."
        ...
        for i in xrange(16):    # 16 building types
            for j in xrange(3): # 3 built eras
                if self.bld[i][j] > 0.:
                    ...
                    # Overwrite some parameters for optimization variables if provided
                    if self.glzR:
                    ...
                    if self.vent:
                        self.BEM[k].building.vent = self.vent
                        self.Sch[k].Vent = self.vent
                    if self.infil:
                        self.BEM[k].building.infil = self.infil
                    if self.shgc:
    ...
    def simulate(self):
        ...
        # Print statement for running simulation
        # print 'Running VCWG for {} days from {}/{}.'.format(int(self.nDay), int(self.Month), int(self.Day))
        ...
        waterHeatConsump_hourly = numpy.zeros(len(Output_TimInd_new))
        ...
        indoorHum_hourly = numpy.zeros(len(Output_TimInd_new))
        ...
        TCanyon_hourly = numpy.zeros(len(Output_TimInd_new))
        qCanyon_hourly = numpy.zeros(len(Output_TimInd_new))
        infil_hourly = numpy.zeros(len(Output_TimInd_new))
        vent_hourly = numpy.zeros(len(Output_TimInd_new))
        window_state_hourly = numpy.zeros(len(Output_TimInd_new))
        ...
        # Window state and infiltration summation variable
        windowstatetotal = 0.0
        infiltrationtotal = 0.0
        for it in range(1,self.simTime.nt,1):# for every simulation time-step defined by uwg
              ...
              for i in xrange(len(self.BEM)):
                  ...
                  # Add from Relative Humidity (RH) schedule for cooling
                  self.BEM[i].building.coolRHSetpointDay = self.Sch[i].RHCool[self.dayType - 1][self.simTime.hourDay]
                  self.BEM[i].building.coolRHSetpointNight = self.BEM[i].building.coolRHSetpointDay
                  # Add from Relative Humidity (RH) schedule for heating
                  self.BEM[i].building.heatRHSetpointDay = self.Sch[i].RHHeat[self.dayType - 1][self.simTime.hourDay]
                  self.BEM[i].building.heatRHSetpointNight = self.BEM[i].building.heatRHSetpointDay
              ...
              self.UCM, self.BEM, dens = urbflux(self.UCM,self.BEM, self.forc, self.geoParam, self.simTime, self.RSM, WindUrban, self.bx, self.by,
                                                 ...
                                                 self.V_pcm, self.l_pcm, self.T_melt, self.rural,self.hvac,self.vx,self.vy,self.dz,self.windowList)
              ...
              # Window state and infiltration compounding
              windowstatetotal = windowstatetotal + self.windowList[0].state
              infiltrationtotal = infiltrationtotal + self.BEM[0].building.infil
              ...
              if it == Output_TimInd_new[iO_new]:
                  ...
                  waterHeatConsump_hourly[iO_new] = self.BEM[0].building.waterHeatConsump
                  ...
                  indoorHum_hourly[iO_new] = self.BEM[0].building.indoorHum
                  ...
                  # Window state and infiltration hourly average
                  window_state_hourly[iO_new] = windowstatetotal / ((60 / self.simTime.dt) * 60)
                  infil_hourly[iO_new] = infiltrationtotal / ((60 / self.simTime.dt) * 60)
        ...
        ProfilesFilename_U = "Output/U_profiles_hourly_{}.txt".format(self.month_name)
        ...
        Values_format_all = Values_format + str('\n')
        ProfilesFilename_V = "Output/V_profiles_hourly_{}.txt".format(self.month_name)
        ...
        Values_format_all = Values_format + str('\n')
        ProfilesFilename_Tr = "Output/Tr_profiles_hourly_{}.txt".format(self.month_name)
        ...
        Values_format_all = Values_format + str('\n')
        ProfilesFilename_Tu = "Output/Tu_profiles_hourly_{}.txt".format(self.month_name)
        ...
        Values_format_all = Values_format + str('\n')
        ProfilesFilename_q = "Output/q_profiles_hourly_{}.txt".format(self.month_name)
        ...
        Values_format_all = Values_format + str('\n')
        ProfilesFilename_TKE = "Output/TKE_profiles_hourly_{}.txt".format(self.month_name)
        ...
        timeseriesFilename = "Output/BEM_hourly_{}.txt".format(self.month_name)
        ...
        for i in range(0,len(Output_TimInd_new)):
            outputFile_BEM.write("%i %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %i %f %f %f \n"
                                 % (i,sensWaste_hourly[i],dehumDemand_hourly[i],humDemand_hourly[i],QWater_hourly[i],QGas_hourly[i],sensCoolDemand_hourly[i],
                                    ...
                                    TCanyon_hourly[i],infil_hourly[i],vent_hourly[i],self.hvac, window_state_hourly[i],
                                    indoorHum_hourly[i],qCanyon_hourly[i]))
        outputFile_BEM.close()

        # T_epw
        timeseriesFilename = "Output/Tepw_hourly_{}.txt".format(self.month_name)
    ...
    def write_epw(self):
        ...
        # Print statement to show that and EPW is generated
